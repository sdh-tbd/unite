...
import { Tag, TagGroup, TagList } from "@unite/ui/components/base/tags/tags"
import { Tooltip, TooltipTrigger } from "@unite/ui/components/base/tooltip/tooltip"
import { cx, sortCx } from "@unite/ui/utils/cx"
...
export function InputTags({
  ...
  const entries = (() => {
    ...
    for (const label of value) {
      ...
      const oldEntry = oldIndex === -1 ? undefined : oldEntries[oldIndex]
      if (oldEntry) {
  ...
  const focusLastTag = useCallback(() => {
    ...
    tagEls?.item(tagEls.length - 1)?.focus()
  }, [])
  ...
  function handleTagGroupKeyDown(event: KeyboardEvent<HTMLDivElement>) {
    if (event.key === "ArrowRight") {
      const tagEls = tagGroupRef.current?.querySelectorAll<HTMLElement>('[role="row"]')
      const lastTag = tagEls?.item(tagEls.length - 1)
      if (lastTag) {
      ...
      >
        {({ isDisabled }) => (
          <>
            ...
            >
              {!isEmpty && (
                // biome-ignore lint/a11y/noStaticElementInteractions: a display-contents wrapper that forwards keys to the TagGroup below, which owns the roles and focus.
