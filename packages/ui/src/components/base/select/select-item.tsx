...
import { CheckboxBase } from "@unite/ui/components/base/checkbox/checkbox"
        ...
        >
          ...
          <div className={cx("flex w-full min-w-0 flex-1 flex-wrap", sizes[size].textContainer)}>
            ...
            {supportingText && (
              <AriaText
                slot="description"
                className={cx("whitespace-nowrap text-tertiary", sizes[size].text)}
              >
                {supportingText}
              </AriaText>
            )}
          ...
          {state.isSelected && selectionIndicator === "checkmark" && (
            <Check
              aria-hidden="true"
              className={cx("ml-auto text-fg-brand-primary", sizes[size].check)}
            />
          )}
