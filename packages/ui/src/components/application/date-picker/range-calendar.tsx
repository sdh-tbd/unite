...
import type { ButtonProps } from "@unite/ui/components/base/buttons/button"
import { Button } from "@unite/ui/components/base/buttons/button"
import { InputDateBase } from "@unite/ui/components/base/input/input-date"
import { useBreakpoint } from "@unite/ui/hooks/use-breakpoint"
...
export function RangeCalendarContextProvider({ children }: PropsWithChildren) {
...
function RangeCalendarTitle({ part }: { part: "start" | "end" }) {
...
interface MobilePresetButtonProps
  extends Omit<ButtonProps, "value" | "slot" | "size" | "color" | "onClick"> {
  value: { start: DateValue; end: DateValue }
}

function MobilePresetButton({ value, children, ...props }: MobilePresetButtonProps) {
  const context = useContext(RangeCalendarStateContext)
